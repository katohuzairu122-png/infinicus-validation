(function(g){"use strict";const r=g.INFINICUS.BO.runtime;const store=g.INFINICUS.BO.customerAccountOperationsEngineStore;

async function registerPolicy(input={}){
  const built=g.INFINICUS.BO.customerAccountOperationsEnginePolicyModel.create(input);
  return built.ok?store.put("policies",built.data):built;
}

async function process(input={}){
  const policy=await store.get("policies",input.customerAccountOperationsEnginePolicyId);
  ...
  const record={
    customerAccountOperationsEngineRecordId:r.createId("bo_record"),
    block:"BO-05",
    purpose:"Manage customer profiles, accounts, contacts, segments, preferences, credit limits, and consent.",
    businessId,
    entityType:String(input.entityType||"customerAccountOperationsEngine"),
  ...
  const event={
    ...
    eventType:String(input.eventType||"bo.customers.register"),
    businessId:record.businessId,
    entityType:record.entityType,
    entityId:record.entityId,
    state:record.state,
  ...
  const handoff={
    salesPipelineHandoffId:r.createId("bo_handoff"),
    sourceBlock:"BO-05",
    targetBlock:"BO-06",
    sourceRecordId:record.customerAccountOperationsEngineRecordId,
    businessId:record.businessId,
  ...
  await r.emit("bo.customers.register.completed",{recordId:record.customerAccountOperationsEngineRecordId,handoffId:handoff.salesPipelineHandoffId});
  return r.success({record,event,handoff});
}

const api=Object.freeze({registerPolicy,process,
getRecord:({customerAccountOperationsEngineRecordId})=>store.get("records",customerAccountOperationsEngineRecordId),
getHandoff:({salesPipelineHandoffId})=>store.get("handoffs",salesPipelineHandoffId),
listRecords:()=>store.list("records"),
listEvents:()=>store.list("events")});
r.registerService("bo.customer_account_operations_engine",api,{block:"BO-05"});

r.registerRoute("bo.customer_policy.register",registerPolicy);
r.registerRoute("bo.customers.register",process);

g.INFINICUS.BO.customerAccountOperationsEngine=api;
})(window);
