(function(g){"use strict";const r=g.INFINICUS.BO.runtime;const schema=[["policies", "fulfilmentDeliveryLogisticsEnginePolicyId"], ["records", "fulfilmentDeliveryLogisticsEngineRecordId"], ["events", "operationalEventId"], ["handoffs", "workforceOperationsHandoffId"]];let dbp;
const req=q=>new Promise((res,rej)=>{q.onsuccess=()=>res(q.result);q.onerror=()=>rej(q.error);});
function open(){if(dbp)return dbp;dbp=new Promise((res,rej)=>{const q=indexedDB.open("INFINICUS_BO_14",1);q.onupgradeneeded=()=>{const d=q.result;for(const [n,k] of schema)if(!d.objectStoreNames.contains(n))d.createObjectStore(n,{keyPath:k});};q.onsuccess=()=>res(q.result);q.onerror=()=>rej(q.error);});return dbp;}
...
g.INFINICUS.BO.fulfilmentDeliveryLogisticsEngineStore=Object.freeze({put,get,list});
})(window);
