(function(g){"use strict";const r=g.INFINICUS.BO.runtime;const store=g.INFINICUS.BO.paymentBillingReceivablesEngineStore;

async function registerPolicy(input={}){
  const built=g.INFINICUS.BO.paymentBillingReceivablesEnginePolicyModel.create(input);
  return built.ok?store.put("policies",built.data):built;
}

async function process(input={}){
  const policy=await store.get("policies",input.paymentBillingReceivablesEnginePolicyId);
  ...
  const record={
    paymentBillingReceivablesEngineRecordId:r.createId("bo_record"),
    block:"BO-09",
    purpose:"Manage invoices, payments, refunds, receivables, reconciliation, overdue balances, and collections.",
    businessId,
    entityType:String(input.entityType||"paymentBillingReceivablesEngine"),
  ...
  const event={
    ...
    eventType:String(input.eventType||"bo.billing.process"),
    businessId:record.businessId,
    entityType:record.entityType,
    entityId:record.entityId,
    state:record.state,
  ...
  const handoff={
    procurementHandoffId:r.createId("bo_handoff"),
    sourceBlock:"BO-09",
    targetBlock:"BO-10",
    sourceRecordId:record.paymentBillingReceivablesEngineRecordId,
    businessId:record.businessId,
  ...
  await r.emit("bo.billing.process.completed",{recordId:record.paymentBillingReceivablesEngineRecordId,handoffId:handoff.procurementHandoffId});
  return r.success({record,event,handoff});
}

const api=Object.freeze({registerPolicy,process,
getRecord:({paymentBillingReceivablesEngineRecordId})=>store.get("records",paymentBillingReceivablesEngineRecordId),
getHandoff:({procurementHandoffId})=>store.get("handoffs",procurementHandoffId),
listRecords:()=>store.list("records"),
listEvents:()=>store.list("events")});
r.registerService("bo.payment_billing_receivables_engine",api,{block:"BO-09"});

r.registerRoute("bo.billing_policy.register",registerPolicy);
r.registerRoute("bo.billing.process",process);

g.INFINICUS.BO.paymentBillingReceivablesEngine=api;
})(window);
