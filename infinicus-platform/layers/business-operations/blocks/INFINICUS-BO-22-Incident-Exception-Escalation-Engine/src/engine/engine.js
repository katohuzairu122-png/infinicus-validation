(function(g){"use strict";const r=g.INFINICUS.BO.runtime;const store=g.INFINICUS.BO.incidentExceptionEscalationEngineStore;

async function registerPolicy(input={}){
  const built=g.INFINICUS.BO.incidentExceptionEscalationEnginePolicyModel.create(input);
  return built.ok?store.put("policies",built.data):built;
}

async function process(input={}){
  const policy=await store.get("policies",input.incidentExceptionEscalationEnginePolicyId);
  ...
  const record={
    incidentExceptionEscalationEngineRecordId:r.createId("bo_record"),
    block:"BO-22",
    purpose:"Manage incidents, exceptions, failures, severity, escalation, remediation, and closure evidence.",
    businessId,
    entityType:String(input.entityType||"incidentExceptionEscalationEngine"),
  ...
  const event={
    ...
    eventType:String(input.eventType||"bo.incidents.manage"),
    businessId:record.businessId,
    entityType:record.entityType,
    entityId:record.entityId,
    state:record.state,
  ...
  const handoff={
    operationalPerformanceHandoffId:r.createId("bo_handoff"),
    sourceBlock:"BO-22",
    targetBlock:"BO-23",
    sourceRecordId:record.incidentExceptionEscalationEngineRecordId,
    businessId:record.businessId,
  ...
  await r.emit("bo.incidents.manage.completed",{recordId:record.incidentExceptionEscalationEngineRecordId,handoffId:handoff.operationalPerformanceHandoffId});
  return r.success({record,event,handoff});
}

const api=Object.freeze({registerPolicy,process,
getRecord:({incidentExceptionEscalationEngineRecordId})=>store.get("records",incidentExceptionEscalationEngineRecordId),
getHandoff:({operationalPerformanceHandoffId})=>store.get("handoffs",operationalPerformanceHandoffId),
listRecords:()=>store.list("records"),
listEvents:()=>store.list("events")});
r.registerService("bo.incident_exception_escalation_engine",api,{block:"BO-22"});

r.registerRoute("bo.incident_policy.register",registerPolicy);
r.registerRoute("bo.incidents.manage",process);

g.INFINICUS.BO.incidentExceptionEscalationEngine=api;
})(window);
