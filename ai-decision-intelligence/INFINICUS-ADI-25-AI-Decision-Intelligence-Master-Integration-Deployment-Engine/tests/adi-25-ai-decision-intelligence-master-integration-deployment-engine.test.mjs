...
await import("../../INFINICUS-ADI-20-Explainability-Evidence-Trace-Reasoning-Engine/src/adi-20-explainability-evidence-trace-reasoning-engine.js");
await import("../../INFINICUS-ADI-21-Next-Best-Action-Recommendation-Generation-Engine/src/adi-21-next-best-action-recommendation-generation-engine.js");
await import("../../INFINICUS-ADI-22-Recommendation-Challenge-Red-Team-Validation-Engine/src/adi-22-recommendation-challenge-red-team-validation-engine.js");
await import("../../INFINICUS-ADI-23-Decision-Gate-Escalation-Human-Review-Engine/src/adi-23-decision-gate-escalation-human-review-engine.js");
await import("../../INFINICUS-ADI-24-Approved-Business-Action-Package-Publication-Handoff/src/adi-24-approved-business-action-package-publication-handoff.js");
await import("../src/adi-25-ai-decision-intelligence-master-integration-deployment-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-25"];
assert.ok(block, "block ADI-25 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createMasterIntegrationEngine, "function");
...
{
  const result = blocks["ADI-21"].attachToADIRuntime(runtime, {});
  assert.equal(result.ok, true, "attach ADI-21 failed: " + JSON.stringify(result.error));
}
{
  const result = blocks["ADI-22"].attachToADIRuntime(runtime, {});
  assert.equal(result.ok, true, "attach ADI-22 failed: " + JSON.stringify(result.error));
}
{
  const result = blocks["ADI-23"].attachToADIRuntime(runtime, {});
  assert.equal(result.ok, true, "attach ADI-23 failed: " + JSON.stringify(result.error));
}
{
  const result = blocks["ADI-24"].attachToADIRuntime(runtime, { publisher: { publish: async () => ({ publicationId: "pub_test" }) } });
  assert.equal(result.ok, true, "attach ADI-24 failed: " + JSON.stringify(result.error));
}
{
  const result = blocks["ADI-25"].attachToADIRuntime(runtime, {});
  assert.equal(result.ok, true, "attach ADI-25 failed: " + JSON.stringify(result.error));
}

// service is registered after the attach chain
const service = runtime.getService("adi.master_integration");
assert.equal(service.ok, true, "service adi.master_integration not registered");
assert.equal(service.data.blockId, "ADI-25");

// master integration reports the full layer ready
const master = runtime.getService("adi.master_integration").data;
const ready = master.assertReady();
assert.equal(ready.ok, true, "assertReady failed: " + JSON.stringify(ready.error));
assert.equal(ready.data.expectedBlocks, 25);
assert.equal(ready.data.missingServices.length, 0);
const plan = master.deploymentPlan();
assert.equal(plan.ok, true);

console.log("ADI-25 tests passed.");
