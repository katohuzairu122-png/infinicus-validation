...
await import("../src/adi-12-decision-objectives-constraints-criteria-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-12"];
assert.ok(block, "block ADI-12 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createFrameworkEngine, "function");
...
const service = runtime.getService("adi.evaluation_framework");
assert.equal(service.ok, true, "service adi.evaluation_framework not registered");
assert.equal(service.data.blockId, "ADI-12");

console.log("ADI-12 tests passed.");
