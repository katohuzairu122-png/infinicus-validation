...
await import("../src/adi-17-risk-opportunity-downside-assessment-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-17"];
assert.ok(block, "block ADI-17 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createRiskAssessmentEngine, "function");
...
const service = runtime.getService("adi.risk_assessment");
assert.equal(service.ok, true, "service adi.risk_assessment not registered");
assert.equal(service.data.blockId, "ADI-17");

console.log("ADI-17 tests passed.");
