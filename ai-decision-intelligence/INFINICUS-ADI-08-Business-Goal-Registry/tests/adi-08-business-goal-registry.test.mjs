...
await import("../src/adi-08-business-goal-registry.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-08"];
assert.ok(block, "block ADI-08 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createGoalRegistry, "function");
...
const service = runtime.getService("adi.goal_registry");
assert.equal(service.ok, true, "service adi.goal_registry not registered");
assert.equal(service.data.blockId, "ADI-08");

console.log("ADI-08 tests passed.");
