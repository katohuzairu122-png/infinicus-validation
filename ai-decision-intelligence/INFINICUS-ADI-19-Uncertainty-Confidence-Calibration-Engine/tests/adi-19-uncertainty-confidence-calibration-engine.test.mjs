...
await import("../src/adi-19-uncertainty-confidence-calibration-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-19"];
assert.ok(block, "block ADI-19 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createConfidenceCalibrationEngine, "function");
...
const service = runtime.getService("adi.confidence_calibration");
assert.equal(service.ok, true, "service adi.confidence_calibration not registered");
assert.equal(service.data.blockId, "ADI-19");

console.log("ADI-19 tests passed.");
