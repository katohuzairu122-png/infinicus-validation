...
await import("../src/adi-11-decision-context-evidence-assembly-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-11"];
assert.ok(block, "block ADI-11 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createAssemblyEngine, "function");
...
const service = runtime.getService("adi.context_evidence_assembly");
assert.equal(service.ok, true, "service adi.context_evidence_assembly not registered");
assert.equal(service.data.blockId, "ADI-11");

console.log("ADI-11 tests passed.");
