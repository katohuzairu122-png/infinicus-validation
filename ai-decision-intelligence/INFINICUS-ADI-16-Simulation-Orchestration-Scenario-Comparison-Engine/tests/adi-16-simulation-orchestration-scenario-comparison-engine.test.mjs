...
await import("../src/adi-16-simulation-orchestration-scenario-comparison-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-16"];
assert.ok(block, "block ADI-16 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createSimulationOrchestrator, "function");
...
const service = runtime.getService("adi.simulation_orchestration");
assert.equal(service.ok, true, "service adi.simulation_orchestration not registered");
assert.equal(service.data.blockId, "ADI-16");

console.log("ADI-16 tests passed.");
