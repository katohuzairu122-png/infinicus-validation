...
await import("../src/adi-09-decision-trigger-registry.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-09"];
assert.ok(block, "block ADI-09 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createTriggerRegistry, "function");
...
const service = runtime.getService("adi.trigger_registry");
assert.equal(service.ok, true, "service adi.trigger_registry not registered");
assert.equal(service.data.blockId, "ADI-09");

console.log("ADI-09 tests passed.");
