...
await import("../src/adi-21-next-best-action-recommendation-generation-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-21"];
assert.ok(block, "block ADI-21 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createRecommendationEngine, "function");
...
const service = runtime.getService("adi.recommendation_generation");
assert.equal(service.ok, true, "service adi.recommendation_generation not registered");
assert.equal(service.data.blockId, "ADI-21");

console.log("ADI-21 tests passed.");
