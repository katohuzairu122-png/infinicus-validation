...
await import("../src/adi-23-decision-gate-escalation-human-review-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-23"];
assert.ok(block, "block ADI-23 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createDecisionGateEngine, "function");
...
const service = runtime.getService("adi.decision_gate");
assert.equal(service.ok, true, "service adi.decision_gate not registered");
assert.equal(service.data.blockId, "ADI-23");

console.log("ADI-23 tests passed.");
