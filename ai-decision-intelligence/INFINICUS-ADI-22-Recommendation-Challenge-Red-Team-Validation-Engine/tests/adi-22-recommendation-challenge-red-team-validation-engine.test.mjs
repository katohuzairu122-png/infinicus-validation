...
await import("../src/adi-22-recommendation-challenge-red-team-validation-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-22"];
assert.ok(block, "block ADI-22 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createRedTeamEngine, "function");
...
const service = runtime.getService("adi.red_team_validation");
assert.equal(service.ok, true, "service adi.red_team_validation not registered");
assert.equal(service.data.blockId, "ADI-22");

console.log("ADI-22 tests passed.");
