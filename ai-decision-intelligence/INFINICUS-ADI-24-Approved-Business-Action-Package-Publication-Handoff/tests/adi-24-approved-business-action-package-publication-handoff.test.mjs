...
await import("../src/adi-24-approved-business-action-package-publication-handoff.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-24"];
assert.ok(block, "block ADI-24 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createABAHandoffEngine, "function");
...
const service = runtime.getService("adi.aba_handoff");
assert.equal(service.ok, true, "service adi.aba_handoff not registered");
assert.equal(service.data.blockId, "ADI-24");

console.log("ADI-24 tests passed.");
