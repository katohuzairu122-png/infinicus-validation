...
await import("../src/adi-18-multi-criteria-decision-scoring-ranking-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-18"];
assert.ok(block, "block ADI-18 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createScoringEngine, "function");
...
const service = runtime.getService("adi.scoring_ranking");
assert.equal(service.ok, true, "service adi.scoring_ranking not registered");
assert.equal(service.data.blockId, "ADI-18");

console.log("ADI-18 tests passed.");
