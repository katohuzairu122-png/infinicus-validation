...
await import("../src/adi-13-strategic-alternative-generation-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-13"];
assert.ok(block, "block ADI-13 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createAlternativeGenerationEngine, "function");
...
const service = runtime.getService("adi.alternative_generation");
assert.equal(service.ok, true, "service adi.alternative_generation not registered");
assert.equal(service.data.blockId, "ADI-13");

console.log("ADI-13 tests passed.");
