...
await import("../src/adi-14-alternative-feasibility-eligibility-filter.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-14"];
assert.ok(block, "block ADI-14 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createFeasibilityFilter, "function");
...
const service = runtime.getService("adi.feasibility_filter");
assert.equal(service.ok, true, "service adi.feasibility_filter not registered");
assert.equal(service.data.blockId, "ADI-14");

console.log("ADI-14 tests passed.");
