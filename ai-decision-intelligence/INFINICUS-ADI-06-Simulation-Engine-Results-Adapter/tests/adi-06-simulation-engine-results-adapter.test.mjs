...
await import("../src/adi-06-simulation-engine-results-adapter.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-06"];
assert.ok(block, "block ADI-06 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createSimulationResultsAdapter, "function");
...
const service = runtime.getService("adi.simulation_results_adapter");
assert.equal(service.ok, true, "service adi.simulation_results_adapter not registered");
assert.equal(service.data.blockId, "ADI-06");

console.log("ADI-06 tests passed.");
