...
await import("../src/adi-15-impact-dependency-trade-off-analysis-engine.js");

const ADI = globalThis.INFINICUS.ADI;
const blocks = ADI.blocks;
const block = blocks["ADI-15"];
assert.ok(block, "block ADI-15 not registered on INFINICUS.ADI.blocks");
assert.equal(typeof block.createImpactAnalysisEngine, "function");
...
const service = runtime.getService("adi.impact_analysis");
assert.equal(service.ok, true, "service adi.impact_analysis not registered");
assert.equal(service.data.blockId, "ADI-15");

console.log("ADI-15 tests passed.");
